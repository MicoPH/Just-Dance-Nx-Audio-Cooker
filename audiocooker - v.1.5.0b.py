...
def Menu():
    ...
    try:
        ...
        conjson1(volume,modepcm,modeopus,channeljson,ffmpegPath,vgaudioPath,vgmstreamPath,0)
    if ver==1.5:
        pass
    else:
        conjson1(volume,modepcm,modeopus,channeljson,ffmpegPath,vgaudioPath,vgmstreamPath,0)
                    ...
                    def startRun():
                        ...
                        if(".ogg" in listfiles or ".opus" in listfiles or ".mp3" in listfiles or ".wav" in listfiles):
                                try:
                                    run((ffmpegPath), stdout=DEVNULL, stderr=STDOUT)
                                    if bitratevorbis==0:
                                        os.system(ffmpegPath+' -y -i "'+listfiles+'"  -acodec libvorbis -ar 48000 -ac '+str(channeljson)+' -filter:a "volume='+volume+'" -map_metadata -1 -loglevel quiet "'+output+'\\'+os.path.splitext(listfiles1)[0]+'.ogg"')
                                    else:
        ...
        def Opt2():
            ...
            try:
                ...
                try:
                    ...
                    if bitratevorbis==0:
                        os.system(ffmpegPath+' -y -i "' + audiofilename +  '"  -acodec libvorbis -ar 48000 -ac '+str(channeljson)+' -filter:a "volume='+volume+'" -map_metadata -1 -loglevel quiet "'+output+'"')
                    else:
    ...
    if(intoption==6):
       ...
       - [Add Feature] Add Refresh Function on Menu''')
            if changever==5:
                print('''     Version 1.4.1 - The New Patch Version
            ...
            if(nbrs==1):
                def Func(nbrs,mode):
                    if(nbrs==1):
                        ...
                        bitbyte=str(input('    Input here from bytes:'))
                    vorbitrate=con["vorbisbitrate"]
                    opubitrate=con["opusbitrate"]
                    if mode=="opus":
                ...
                def GoFunc(mode):
                    print('''    Choose the bitrate here:
    (Note: 192k = 192000)
    [1] Auto
    [2] 32000 (Low)
    [3] 96000 (Medium)
    [4] 128000 (Mild High)
    [5] 256000 (High) (best for cooked opus)
    [6] 320000 (High) (best for ogg vorbis)
    [7] Custom
    ''')
                    nbrs=int(input('    Choose the choices here --> '))
                    Func(nbrs,mode)
                def O10():
                    print('''    Choose Bitrate Type:
            [1] Cooked Opus
            [2] Ogg''')
                    nbrs = int(input('Choose the enc bit here --> '))
                    if nbrs==1:
                        GoFunc("opus")
                    if nbrs==2:
                        GoFunc("vorbis")
            ...
            if(nbrs==2):
                ...
                def O10():
                    print('''    Choose Path Environment Type:
            [1] FFMPEG
            [2] VGMStream
            [3] VGAudio''')
            if(nbrs==3):
            ...
            if(nbrs==5):
                def Func(nbrs):
                    if nbrs==1:
                        def GoFunc(nbrs):
                            if(nbrs==1):
                                print('''    Choose Mode from Cooked PCM:
